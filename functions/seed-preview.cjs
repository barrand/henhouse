...
async function seed() {
  ...
  await db.doc('games/preview-lobby/players/p1').set({ id: 'p1', name: 'Alice', connected: true, eggs: 0 })
  ...
  await db.doc('games/preview-quest/players/p1').set({ id: 'p1', name: 'Alice', connected: true, eggs: 2 })
  ...
  await db.doc('games/preview-revl/players/p1').set({ id: 'p1', name: 'Alice', connected: true, eggs: 3 })
  ...
  await db.doc('games/preview-revl/rounds/3').set({
    ...
    answerGroups: [],    // flat - no nested arrays
    flockAnswer: ['Gift'],
  ...
  await db.doc('games/preview-score').set({
    ...
    hostId: 'p1', originalHostId: 'p1', status: 'playing', currentRound: 10,
    playerIds: ['p1', 'p2', 'p3', 'p4'],
    settings: { totalRounds: 10, secondsPerRound: 60 },
    rottenEggHolder: 'p2', categories: [], includePatrioticQuestions: false,
  })
  await db.doc('games/preview-score/players/p1').set({ id: 'p1', name: 'Alice', connected: true, eggs: 6 })
