...
async function seed() {
  // QUESTION ANSWERING - me as p1 (host), haven't answered yet
  ...
  await db.doc('games/preview-score').set({
    ...
    hostId: ME, originalHostId: ME, status: 'playing', currentRound: 10,
    playerIds: [ME, 'p2', 'p3', 'p4'],
    settings: { totalRounds: 10, secondsPerRound: 60 },
  ...
  // Also update LOBBY to include me
  await db.doc('games/preview-lobby').update({ playerIds: [ME, 'p2', 'p3'], hostId: ME })
