...
export default function RevealView({ game, round, players, isHost, currentPlayerId }: Props) {
  ...
  const correctAnswer = round.correctAnswer ?? 'truth'
  ...
  const answerGroups: Record<TruthOrTurdAnswer, string[]> = { truth: [], turd: [] }
  const noAnswerIds: string[] = []
  for (const player of players) {
    const answer = round.playerAnswers?.[player.id]
    if (answer === 'truth' || answer === 'turd') answerGroups[answer].push(player.id)
    else noAnswerIds.push(player.id)
  }

  const playerNameById = (id: string) => players.find((player) => player.id === id)?.name ?? id
  const renderNameChips = (playerIds: string[], answer?: TruthOrTurdAnswer) => (
  ...
  const hero = (() => {
    ...
    if (yourResult === 'no-answer') {
      return {
        ...
        detail: `Correct was ${ANSWER_LABEL[correctAnswer]}.`,
        points: null,
  ...
  return (
    <main className="flex-1 px-4 py-6 space-y-5">
      <section className="rounded-2xl border-2 p-5 text-center shadow-sm bg-surface-container-lowest border-outline-variant/60">
        <p className="font-headline text-xl font-bold text-on-surface leading-relaxed">{round.statement}</p>
      </section>
      ...
      <section className="rounded-2xl border-2 border-primary bg-primary-fixed p-5 text-center shadow-sm">
        <p className="font-label text-[10px] uppercase tracking-[0.2em] font-bold text-on-primary-fixed-variant mb-1">
          Correct answer
        </p>
        <p className="font-headline text-4xl font-bold text-on-primary-fixed">{ANSWER_LABEL[correctAnswer]}</p>
        {round.explanation && (
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 mb-2.5">
                ...
                <p className="font-headline text-base font-bold flex-1 text-on-surface">{ANSWER_LABEL[answer]}</p>
              ...
              {ids.length > 0 ? renderNameChips(ids, answer) : (
